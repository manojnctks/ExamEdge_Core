...
namespace ExamEdge.Utils
{
    public class Util : IDisposable
    {
        ...
        public List<SelectListItem> GetOrganizationList(string selectedId)
        {
            List<SelectListItem> list = new List<SelectListItem>();
            list = (from t1 in db.Organizations
                    select new SelectListItem
                    {
                        Value = t1.OrganizationId,
                        Text = t1.OrganizationName,
                        Selected = t1.OrganizationId == selectedId ? true : false
                    }).ToList();
            return list;
        }
        //public UserProfileViewModel GetUserProfile(string userId)
        //{
        //    (from t1 in db.UserProfiles
        //     join t2 in db.Branches on t1.RefId
